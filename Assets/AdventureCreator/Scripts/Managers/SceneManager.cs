...
namespace AC
{
	...
	public class SceneManager : ScriptableObject
	{
		...
		private void ShowSettingsGUI ()
		{
			...
			if (showCutscenes)
			{
				KickStarter.sceneSettings.actionListSource = (ActionListSource) CustomGUILayout.EnumPopup ("ActionList source:", KickStarter.sceneSettings.actionListSource, "AC.KickStarter.sceneSettings.actionListSource");
				...
				{
					KickStarter.sceneSettings.actionListAssetOnStart = (ActionListAsset) CustomGUILayout.ObjectField <ActionListAsset> ("On start:", KickStarter.sceneSettings.actionListAssetOnStart, true, "AC.KickStarter.sceneSettings.actionListAssetOnStart");
				}
				EditorGUILayout.EndHorizontal ();
				EditorGUILayout.BeginHorizontal ();
				if (KickStarter.sceneSettings.actionListSource == ActionListSource.InScene)
				...
				{
					KickStarter.sceneSettings.actionListAssetOnLoad = (ActionListAsset) CustomGUILayout.ObjectField <ActionListAsset> ("On load:", KickStarter.sceneSettings.actionListAssetOnLoad, true, "AC.KickStarter.sceneSettings.actionListAssetOnLoad");
				}
				EditorGUILayout.EndHorizontal ();
				EditorGUILayout.BeginHorizontal ();
				if (KickStarter.sceneSettings.actionListSource == ActionListSource.InScene)
				...
				{
					KickStarter.sceneSettings.actionListAssetOnVarChange = (ActionListAsset) CustomGUILayout.ObjectField <ActionListAsset> ("On variable change:", KickStarter.sceneSettings.actionListAssetOnVarChange, true, "AC.KickStarter.sceneSettings.actionListAssetOnVarChange");
				}
				EditorGUILayout.EndHorizontal ();
			}
					...
					if (AdvGame.GetReferences ().settingsManager.sceneAttributes.Count > 0)
					{
						...
						if (KickStarter.sceneSettings.attributes.Count > 0)
						{
							foreach (InvVar attribute in KickStarter.sceneSettings.attributes)
							{
								...
								{
									label = "Property " + attribute.id.ToString () + ":";
								}
								...
								if (attribute.type == VariableType.Boolean)
								{
									...
									string[] boolType = {"False", "True"};
									attribute.val = CustomGUILayout.Popup (label, attribute.val, boolType, apiPrefix + ".GetProperty (" + attribute.id + ").val");
								}
								else if (attribute.type == VariableType.Integer)
								{
									attribute.val = CustomGUILayout.IntField (label, attribute.val, apiPrefix + ".GetProperty (" + attribute.id + ").val");
								}
								else if (attribute.type == VariableType.PopUp)
								{
									attribute.val = CustomGUILayout.Popup (label, attribute.val, attribute.popUps, apiPrefix + ".GetProperty (" + attribute.id + ").val");
								}
								else if (attribute.type == VariableType.String)
								{
									attribute.textVal = CustomGUILayout.TextField (label, attribute.textVal, apiPrefix + ".GetProperty (" + attribute.id + ").textVal");
								}
								else if (attribute.type == VariableType.Float)
								{
									attribute.floatVal = CustomGUILayout.FloatField (label, attribute.floatVal, apiPrefix + ".GetProperty (" + attribute.id + ").floatVal");
								}
							}
